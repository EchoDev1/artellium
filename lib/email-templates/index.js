...
export function generatePaymentReceiptEmailHtml({ 
  ...
  const formattedTotal = currency === 'USD' 
    ? `$${Math.round(totalAmount / rate).toLocaleString()}` 
  ...
  const itemsRows = (items.length > 0 ? items : [{ title: 'Masterpiece Artwork Acquisition', artistName: 'Pan-African Master', price: totalAmount }]).map(it => `
    <tr>
      ...
      <td align="right" style="padding: 14px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-weight: 700; color: #FAF8EE; font-size: 14px;">
        ${currency === 'USD' ? `$${Math.round((it.price || totalAmount) / rate).toLocaleString()}` : `₦${Number(it.price || totalAmount).toLocaleString()}`}
      </td>
    </tr>
  `).join('');
...
export function generateArtistSaleEmailHtml({
  ...
  const rate = (usdExchangeRate && usdExchangeRate > 0) ? usdExchangeRate : 1480;
  const formattedGross = currency === 'USD' ? `$${Math.round(grossAmount / rate).toLocaleString()}` : `₦${Number(grossAmount).toLocaleString()}`;
  const formattedNet = currency === 'USD' ? `$${Math.round(netPayout / rate).toLocaleString()}` : `₦${Number(netPayout).toLocaleString()}`;
  const formattedFee = currency === 'USD' ? `$${Math.round(platformFee / rate).toLocaleString()}` : `₦${Number(platformFee).toLocaleString()}`;
...
export function generateOutbidAlertEmailHtml({
  ...
  const rate = (usdExchangeRate && usdExchangeRate > 0) ? usdExchangeRate : 1480;
  const formattedCurrent = currency === 'USD' ? `$${Math.round(currentBid / rate).toLocaleString()}` : `₦${Number(currentBid).toLocaleString()}`;
  const formattedPrev = currency === 'USD' ? `$${Math.round(previousBid / rate).toLocaleString()}` : `₦${Number(previousBid).toLocaleString()}`;
...
export function generateBidConfirmationEmailHtml({
  ...
  const rate = (usdExchangeRate && usdExchangeRate > 0) ? usdExchangeRate : 1480;
  const formattedBid = currency === 'USD' ? `$${Math.round(bidAmount / rate).toLocaleString()}` : `₦${Number(bidAmount).toLocaleString()}`;
