...
export const resend = new Resend(RESEND_API_KEY || 're_dummy_key');

// Sender identity: Strictly use verified custom domain notifications@artellium.africa
export const DEFAULT_FROM_EMAIL = process.env.RESEND_FROM_EMAIL || 'Artellium Africa <notifications@artellium.africa>';
export const DOMAIN_FROM_EMAIL = 'Artellium Africa <notifications@artellium.africa>';
...
export async function sendEmail({ to, subject, html, text, from = DEFAULT_FROM_EMAIL }) {
  ...
  // Ensure 'from' uses verified domain
  const sender = from && !from.includes('resend.dev') ? from : 'Artellium Africa <notifications@artellium.africa>';
 ...
 */
export async function sendPaymentConfirmationEmail({ to, name, orderId, paymentReference, items, totalAmount, currency, usdExchangeRate, settlementBank }) {
  const html = generatePaymentReceiptEmailHtml({ name, orderId, paymentReference, items, totalAmount, currency, usdExchangeRate, settlementBank });
  return sendEmail({
    to,
 ...
 */
export async function sendArtistSaleAlertEmail({ to, artistName, buyerName, artworkTitle, grossAmount, netPayout, platformFee, currency, usdExchangeRate, orderId }) {
  const html = generateArtistSaleEmailHtml({ artistName, buyerName, artworkTitle, grossAmount, netPayout, platformFee, currency, usdExchangeRate, orderId });
  return sendEmail({
    to,
 ...
 */
export async function sendOutbidAlertEmail({ to, name, lotNumber, artworkTitle, artistName, currentBid, previousBid, currency, usdExchangeRate, auctionUrl }) {
  const html = generateOutbidAlertEmailHtml({ name, lotNumber, artworkTitle, artistName, currentBid, previousBid, currency, usdExchangeRate, auctionUrl });
  return sendEmail({
    to,
 ...
 */
export async function sendBidConfirmationEmail({ to, name, lotNumber, artworkTitle, artistName, bidAmount, currency, usdExchangeRate, auctionUrl }) {
  const html = generateBidConfirmationEmailHtml({ name, lotNumber, artworkTitle, artistName, bidAmount, currency, usdExchangeRate, auctionUrl });
  return sendEmail({
    to,
