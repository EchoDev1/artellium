      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 sm:w-[410px] rounded-2xl bg-[#090b10] border border-art-gold/40 shadow-2xl z-50 overflow-hidden animate-fade-in backdrop-blur-2xl">
          ...
          {isLoggedIn && currentUser ? (
            ...
            <div className="p-4 sm:p-5 space-y-4 max-h-[85vh] overflow-y-auto scrollbar-thin">
              ...
              {authTab === 'signup' && (
                <form onSubmit={handleSignup} className="space-y-3 text-xs">
                  ...
                  {cloudflareVerified ? (
                    <div className="flex items-center justify-center gap-2 py-1.5 px-3 bg-emerald-950/80 border-2 border-emerald-400 rounded-xl text-emerald-200 text-xs font-bold shadow-[0_0_18px_rgba(16,185,129,0.35)] animate-fade-in text-center">
                    ...
                    <div className="flex items-center justify-center gap-1.5 py-1 px-3 bg-white/[0.03] border border-white/10 rounded-xl text-[10.5px] text-amber-300/80 font-medium text-center">
                  ...
                  <div ref={submitBtnRef} className="pt-1.5 pb-0.5 w-full">
                    <button
                      type="submit"
                      disabled={isLoading}
                      style={{ minHeight: '56px' }}
                      className={`w-full py-3.5 px-5 rounded-2xl font-black text-sm uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2.5 cursor-pointer disabled:opacity-60 relative overflow-hidden group shadow-2xl ${
                        cloudflareVerified
                          ? 'bg-gradient-to-r from-[#FFF59D] via-[#FFD700] to-[#F59E0B] text-black border-2 border-amber-200 shadow-[0_0_35px_rgba(255,215,0,0.9)] scale-[1.02] ring-4 ring-amber-300/70 animate-create-account-pulse'
